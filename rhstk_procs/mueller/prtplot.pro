pro prtplot, nrps, windownr, indx, a, muellerparams1, $
	...
	title=title, $
	path=path, plotfilename=plotfilename, vsrc=vsrc, sigvsrc=sigvsrc
...
yot=yot-dyot
xyouts, xot, yot, (keyword_set(PS) ? 'POL!DSRC!N = ' : 'VSRC = ') + $
        strtrim(string(vsrc, format='(f+6.3)'),2) + $
        ;pmstr + string(0., format='(f6.3)'), /norm, FONT=font
        pmstr + string(sigvsrc, format='(f6.3)'), /norm, FONT=font

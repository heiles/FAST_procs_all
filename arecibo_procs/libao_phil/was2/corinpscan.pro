...
;                         han=han,df=df,hdronly=hdronly,_extra=extra)
;
; ARGS:
...
;     hdronly: if set then only return the header values, no data..
;RETURNS:
...
;21nov04 was not testing if keyword set for hdronly.
;02arp06 if not aligned on scan boundary (and no position) then error out.
;     
function corinpscan,lun,b,brecs,sum=sum,maxrecs=maxrecs,scan=scan,$
                    sl=sl,dbg=dbg,han=han,df=df,hdronly=hdronly,_extra=extra
;
;
...
;    on_error,2  <FIX!!!>
    ...
    if not keyword_set(maxrecs)  then maxrecs=600
    if maxrecs le 0 then maxrecs=600
    maxrecsL=maxrecs
    if not keyword_set(dbg) then dbg=0
    dosum=keyword_set(sum)
    if usewas then curpos=lun.curpos
    if keyword_set(scan) then begin
        if usewas then begin
            istat=waspos(lun,scan,1)
            if istat eq 1 then curpos=lun.curpos
        endif else begin
    ...
    endelse
;
;   if was find scan index, get number of records
;
    if usewas then begin
        ind=where(lun.curpos eq lun.scanI.rowstartind,count)
        if count gt 0 then begin
            nrecs=lun.scanI[ind[0]].recsinscan
            if nrecs eq 0 then begin
               print,'No complete recs in scan',lun.scanI[ind[0]].scan
               goto,errinp
            endif
            maxrecsL=nrecs
        endif else begin
               print,'corinpscan:not aligned on a scan boundary:' 
               goto,errinp
        endelse
    endif
    if dbg then cumread=0.D
    limit=9999
    if ((not dosum) or (usebrecs)) then limit=maxrecsL
    hitEof=0
...
; 
      ...
      if usewas then begin
          curpos=lun.curpos
          istat=wasget(lun,bl,han=han,hdronly=hdronly,_extra=extra)
      endif else begin  
          point_lun,-lun,curpos
          istat=corget(lun,bl,han=han,_extra=extra)
...
;
      if (grp eq 0) then begin
        ...
        if ( dosum ) then begin
            b=bl
            if (usebrecs) then brecs=corallocstr(bl,maxrecsL)
        endif else begin 
            b=corallocstr(bl,maxrecsL)
        endelse
      endif
      ...
      if dosum then begin
         if grp ne 0 then begin
            for i=0,nbrds-1 do begin
                if not keyword_set(hdronly) then b.(i).d=b.(i).d + bl.(i).d    
    ...
    if dosum then begin
        scl=1./grp
        for i=0,nbrds-1 do begin
                if not keyword_set(hdronly) then b.(i).d=b.(i).d*scl
        ...
        if (grp ne maxrecsL) and (usebrecs) then begin
            brecs=brecs[0:grp-1]
        endif
    endif else begin
        if grp ne maxrecsL then begin
            b=b[0:grp-1]
        endif
    endelse

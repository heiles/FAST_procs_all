pro gfitflex_exp_wgt, look, xdataa, tdataa, wgtdataa, $
	xrange, zro0, hgt0, cen0, wid0, $
...
;	in the fit. Weighted fit; set weights equal to delta_tdata to get 
;	a chi-squared fit.
;
;CALLING SEQUENCE:
;    GFITFLEX_EXP_WGT, look, xdataa, tdataa, wgtdataa, $
;	xrange, zro0, hgt0, cen0, wid0, $
;	zro0yn, hgt0yn, cen0yn, wid0yn, $
;	tfita, sigma, zro1, hgt1, cen1, wid1, $
;	sigzro1, sighgt1, sigcen1, sigwid1, cov, problem
...
;     wgtdata: the weights of the data points.
...
;     chisq: the reduced chi sq of the fit, assuming that...
;	wgtdataa = 1./sigma(tdataa) where sigma(tdata) is the rms error
;	in the input tdata array.
;
;RESTRICTIONS:
...
datasize = 0l
for nnr = 0, nr_of_ns-1 do $
	datasize = datasize + xrange[2*nnr+1]-xrange[2*nnr]+1l
xdata = fltarr(datasize,/nozero)
tdata = fltarr(datasize,/nozero)
wgtdata = fltarr(datasize,/nozero)
...
wgtdata[dtsiz:dtsiz1-1] = wgtdataa[ xrange[2*nnr]:xrange[2*nnr+1] ]
dtsiz = dtsiz1
endfor
...
;MULTIPLY S BY THE WEIGHTS...
for npar= 0, nparams-1 do s[ npar,*]= s[ npar,*]* wgtdata

;STOP

;CREATE AND SOLVE THE NORMAL EQUATION MATRICES...
;t = tdata-t_predicted
t = wgtdata* (tdata-t_predicted)
ss = transpose(s) ## s
...
chisq = total( (resid*wgtdata)^2)/(datasize - nparams)

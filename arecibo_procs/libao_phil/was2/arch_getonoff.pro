...
;
            ...
            if Find ne curFileInd then begin
                if keyword_set(desc) then wasclose,desc
                desc=''
                istat=wasopen(slfileAr[find].path + slfilear[find].file,desc)
                curfileind=find
            endif
            desc.curpos=slAr[i_sltouse].rowstart  ; position to start of scan
            istat=corposonoff(desc,bloc,t,sclcal=sclcal,scljy=scljyl,han=han,$
                          maxrecs=maxrecs)

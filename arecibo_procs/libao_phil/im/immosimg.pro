...
;
        plot,immkfrq(dfrq.r[0]),tm,xtitle=xtitle,ytitle=ytitle, $
                xstyle=1,ystyle=1,/nodata,_extra=e,chars=cs
;
;       tsys label
;
...
;			    hist_equal blows up if constant passed in
				if (minv eq maxv) then return,255B + byte(datAr-minv)

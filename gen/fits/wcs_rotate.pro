...
;               [LONGPOLE = , LATPOLE = , /REVERSE, THETA0 = ]
;
; INPUT PARAMETERS:
...
;       ORIGIN keyword no longer used, replaced by THETA0 keyword somewhere
;       along the way but never documented.  Added documentation.
;               T. Robishaw Sep 2006
;       
;-
...
; check to see that enough parameters (we need 5) were sent
 if (N_params() ne 5) then begin
    print,'Syntax - WCS_ROTATE, longitude, latitude, phi, theta, crval,'
    print,'             THETA0 = , [LATPOLE =,  LONGPOLE = , /REVERSE ]'
    return
 endif 
...
; standard system (default = 180 degrees).
 ...
 ; Aug 2006: Whoever added the THETA0 keyword (a) didn't document it and
 ;           (b) made it a keyword when it is a required input paremeter.
 ;           They never checked for its existence, so we do so here.
 if (N_elements(theta0) eq 0) then $
    message, 'Keyword THETA0 must be set. If unsure about its value, '+$
             'refer to Table 13 of Calabretta & Greisen 2002, A&A, 395, 1077.'

; If Theta0 = 90 then CRVAL gives the coordinates of the origin in the native
; system.  This must be converted (using Eq. 7 in Calabretta & Greisen with
; theta0 = 90) to give the coordinates of the North pole (alpha_p, delta_p)

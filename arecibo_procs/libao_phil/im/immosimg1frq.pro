...
;singleCol:         force single col output
;               
...
pro immosimg1frq,year,mon,day1,dohardcopy,_extra=e,freq=freq,nplts=nplts,$
            bpc=bpc,win=win,file=filename,singleCol=singleCol
;
...
;
	title='Arecibo Obs hilltop monitor rfi time vs Freq images'
	use1col=keyword_set(singleCol)
    ...
    immossetup,0,2,dohardcopy,mos,file=filename
	if keyword_set(use1col) then begin
		mos.numcols=1
		mos.numrows=nplts
	endif else begin
    ...
    for i=0,pltperpage-1 do begin
		...
		if nfound lt 2 then begin
			print,"yymmdd:",yymmdd," less than 2 spectra for freq",freq
			continue
		endif
        if (bpc ne 0 ) then imbpc1,dfrq
        immosimg1,dfrq,curcol,currow,mos,dohardcopy,_extra=e,cs=cs
        ...
        if  (i mod pltperpage) eq 0 then begin
            xyouts,.5,mos.yOffMainTitle,title,alignment=.5,/normal
        endif
    endfor

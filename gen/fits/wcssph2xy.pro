...
;               coordinates lie.
;       PV2 - vector of projection parameters, containing, as necessary,
;             [PV2_1,PV2_2,PV2_3,...,PV2_N]; see table above to determine
;             how many (if any) parameters are required for the map
...
;       3.3     Jul 2006 T. Robishaw fixed problems introduced with PV2
;                   keyword; used to accept PV2_1 and PV2_2 keywords, then
;                   compacted to PV2 keyword, a vector containing [PV2_1,
;                   PV2_2,...], but code and documentation were not fixed
;                   after this change.  Now it has been fixed.  Also
;                   fixed poor use of keyword_set() instead of n_elements()
;                   that was causing useless error messages.  And error
;                   checking of CTYPE keyword when MAP_TYPE is set.
;-
...
; check to see that enough parameters (at least 4) were sent
 if (N_params() lt 4) then begin
    ...
    print,'           CTYPE=, FACE=, PV2=, CRVAL=, CRXY=, LATPOLE='
...
; check to see that the data arrays have the same size
 ...
 if (N_params() eq 5) then begin
  ...
  if keyword_set(ctype) then message,$
     'Use either the MAP_TYPE positional parameter or set the projection '+$
     'type with CTYPE, but not both.'
  if ((map_type ge 0) and (map_type le 26)) then begin
      projection_type=map_types[map_type]
  endif else message,'MAP_TYPE must be >= 0 and <= 26, it was set to '+$
                     strtrim(map_type,2)
...
; point.  As suggested in 'Representations of Celestial Coordinates in
; FITS' the default type is set to CAR (Cartesian) the simplest of all
; projections.
...
; PULL OUT KEYWORDS PV2_1 AND PV2_2 FROM INPUT PV2 KEYWORD...
; IF KEYWORD ISN'T SET, WE'LL USE THE DEFAULT VALUES OF ZERO; IF A
; PROJECTION REQUIRES DIFFERENT DEFAULTS, THOSE WILL BE SET BELOW. 
...
; of program.  If the x and y offsets are not set then assume they are
; zero.
...
; Make small offsets at poles to allow the transformations to be completely
; invertible.  These generally introduce a small fractional error but only
; at the poles.  They are necessary since all maps lose information at the
; poles when a rotation is applied, because all points within NORTH_ or
; SOUTH_OFFSET of the poles are mapped to the same points.
...
; Convert from standard coordinate system to "native" coordinate system if
; the CRVAL keyword is set.  Otherwise, assume the latitude and longitude
; given are in "native" coordinates already (this is essentially what is
; done in the procedure AITOFF).
...
case strupcase(projection_type) of
   ...
   'SZP': begin
      mu = PV2_1
      phi_c = PV2_2
      theta_c = N_elements(PV2) GT 2 ? PV2[2] : 90
   ...
   'AIR':begin
      if (N_elements(PV2) eq 0) then begin
         message,/informational,$
                 'PV2_1 not set, using default of PV2_1 = 90 for '+$
                 'AIR map projection'
         PV2_1 = 9.d1
      endif
      theta_b = PV2_1/radeg
      ...
      ; When theta_b (aka PV2_1 in radians) is equal to pi/2 the normal
      ; equations for the AIR projection produce infinities.  To avoid the
      ; problem, values of theta_b equal to pi/2 cause a different set of
      ; equations to be used.
      if (theta_b eq pi2) then begin

         ; AIR produces the same radii for different latitudes, causing
         ; some overlap.  To avoid this problem, if latitudes which are far
         ; enough south to be a problem are included in the data, the
         ; routine will stop.
         if (min(theta) lt -36/radeg) then begin
            message, 'AIR produces overlap of native latitudes south of '+$
                     '-36 with the PV2_1 = 90', /INFO
            return
         endif

         ; points with xi too small are labelled as bad to prevent poor
         ; behavior of the equation for r_theta
         ...
         if (i lt (n_elements(radius)- 2)) $
            then min_lat = 90 - 2*radeg*xi_temp[i] $
         else min_lat = -90
         if (min(theta) lt min_lat[0]/radeg) then begin
            message,'AIR produces overlap of native latitudes south of '+$
                    string(min_lat[0],' with the PV2_1 = ',PV2_1,$
                           FORMAT='(I3,A,I3)'), /INFO
            return
         endif
         ...
         r_theta = lng*0
         if (Ngood GT 0) $
            then r_theta[good] = -2*radeg*(alog(cos(xi[good]))/$
   ...
   'CYP':begin
      if (N_elements(PV2) EQ 0) then $
         message, 'PV2_1 not set, using default of PV2_1 = 0 for '+$
                  'CYP map projection', /INFO
      if (N_elements(PV2) EQ 1) then begin
         message, 'PV2_2 not set, using default of PV2_2 = 1 for '+$
                  'CYP map projection', /INFO
         PV2_2 = 1.d0
      endif
   ...
   'CEA':begin
      ; DEFAULT VALUE IF NOT SET IS FOR LAMBERT'S PROJECTION...
      if (N_elements(PV2) eq 0) then begin
         message, 'PV2_1 not set, using default of PV2_1 = 1 for '+$
                  'CEA map projection.', /INFO
         PV2_1 = 1d0
      endif
   ...
   'COP':begin
      if (N_elements(PV2) eq 0) then message, $
         'COP map projection requires that PV2_1 keyword be set.'
      if (N_elements(PV2) lt 2) then message, /INFO, $
         'PV2_2 not set, using default of PV2_2 = 0 for COP map projection'
      if ((PV2_1 lt -90) or (PV2_2 gt 90) or (PV2_1 gt 90)) then message,$
         'PV2_1 and PV2_2 must satisfy -90<=PV2_1<=90,0<=PV2_2<=90 '+$
         'for COP projection'
      if (PV2_1 eq -PV2_2) then message,$
         'COP projection with PV2_1=-PV2_2 is better done as a '+$
         'cylindrical projection'
      theta_a = PV2_1/radeg
      alpha = PV2_2/radeg
      bad = where((theta ge theta_a + pi2) or (theta le theta_a - pi2))
      if (bad[0] ne -1) then $
         message, 'COP map projection diverges for native latitude = '+$
                  'PV2_1 +/- 90. Remove these points and try again.'
   ...
   'COD':begin
      ...
      if ((PV2_1 lt -90) or (PV2_2 gt 90) or (PV2_1 gt 90)) then message,$
         'PV2_1 and PV2_2 must satisfy -90<=PV2_1<=90,PV2_2<=90 '+$
         'for COD projection'
      if (PV2_1 eq -PV2_2) then message,$
      ...
      if (PV2_2 NE 0) then begin
         ...
         ; if the two parameters PV2_1 and PV2_2 are equal use the simpler
         ; set of equations
   ...
   'COE':begin
      ...
      if ((PV2_1 lt -90) or (PV2_2 gt 90) or (PV2_1 gt PV2_2)) then message,$
         'PV2_1 and PV2_2 must satisfy -90<=PV2_1<=PV2_2<=90 for '+$
         'COE map projection'
      if (PV2_1 eq -PV2_2) then message,$
   ...
   'COO':begin
      ...
      if ((PV2_1 lt -90) or (PV2_2 gt 90) or (PV2_1 gt 90)) then message,$
         'PV2_1 and PV2_2 must satisfy -90<=PV2_1<=90,PV2_2<=90 for '+$
         'COO projection'
      if (PV2_1 eq -PV2_2) then message,$
      ...
      y_0 = alpha*tan((pi2-theta_a)/2d0)^c 
      a_phi = c*phi
      x = r_theta*sin(a_phi)
      y = y_0 - r_theta*cos(a_phi)
      ;stop
   ...
   'BON':begin
      ...
      if (PV2_1 eq 0) then message,$
         'PV2_1 = 0 for BON map projection is better done with SFL '+$
         'map projection'
   ...
   'PCO':begin
      ; The equations for x and y are poorly behaved for theta = 0.  Avoid
      ; this by explicitly assigning values for x and y when theta = 0.
      ...
      if Ngood GT 0 then begin
         ...
         y[good_ind] = radeg*(theta[good_ind]+$
                              (1.d0 - cos(phi[good_ind]*sin(theta[good_ind])))$
                              / tan(theta[good_ind]))
      endif
   end
   ...
   'CSC':begin
      ...
      ; determine the face on which the x and y coordinates will reside by
      ; setting rho equal to the maximum of n,m,l,-m,-l,-n which
      ; corresponds to faces 0 through 5 respectively
      rho =  lng*0
      if size(lng,/N_dimen) EQ 0 then  face = 0 else face = lonarr(n_long)
      
      ; use an array to store a remapping of the direction cosines.  This
      ; way, faces 0 and 5 take points on their borders with faces 1-4.
      ; The reason for this is that if the max function sees identical
      ; values in an array, it takes the index of the first occurrence of
      ; that value.
      remap = [0,5,2,1,4,3]
      ...
      ; based on the face determined for each point, find the parameters
      ; alpha and beta1
      alpha = lng*0
      beta1 = alpha
      for i = 0l, n_long-1 do begin
      ...
      x = fconst*(alpha*gam_s+alpha^3*(1-gam_s)+$
                  alpha*beta1^2*(1-alpha^2)*$
                  (gam+(em-gam)*alpha^2+(1-beta1^2)*$
                   (c_00+c_10*alpha^2+c_01*beta1^2+$
                    c_20*alpha^4+c_11*alpha^2*beta1^2+c_02*beta1^4))+$
                  alpha^3*(1-alpha^2)*$
                  (ome-(1-alpha^2)*(d_0+d_1*alpha^2)))
      y = fconst*(beta1*gam_s+beta1^3*(1-gam_s)+$
                  beta1*alpha^2*(1-beta1^2)*$
                  (gam+(em-gam)*beta1^2+(1-alpha^2)*$
                   (c_00+c_10*beta1^2+c_01*alpha^2+$
                    c_20*beta1^4+c_11*beta1^2*alpha^2+c_02*alpha^4))+$
                  beta1^3*(1-beta1^2)*$

pro z17_2, npatt, blchnls, hdrsrcname, hdrscan, hdr1info, hdr2info, $
            stkon, stk16offs, z17_2, ncov=ncov, trxoffs=trxoffs, cont=cont
  
;+       
;PURPOSE:Derive expected profiles and optical depths from z17 absorption
;data, going to second derivatives in ra and dec. The equations of
;condition for each channel are:
;  (Tobs - Trcvr) = Tcont exp^{-tau} + Texp +
;          delra * dTexp/dra + deldec * dTexp/ddec +
;          0.5*delra^2 * d^2Texp/dra^2 + 0.5*deldec^2 * d^2Texp/ddec^2 +
;          delra*deldec* d^2Texp/dra*ddec
;where;
...
;  Kelvins/arcmin
;  d^2Texp/dra^2 is the spatial second derivative of Texp in ra 
;        (not corrected for cos (dec); units Kelvins/arcmin
;  d^2Texp/ddec^2 is the spatial second derivative of Texp in dec
;  d^2Texp/dra ddec is the spatial second derivative of Texp in ra*dec
;
;CALLING SEQUENCE:
;  z17_2, npatt, blchnls, hdrsrcname, hdrscan, hdr1info, hdr2info, $
;            stkon, stk16offs, z17_2, ncov=ncov, trxoffs=trxoffs, cont=cont
;
;INPUTS
...
;       CONT, the continuum source defln obtaained from blchnls
;
...
;        dT2exp_dra2:  array of 2048 2nd derivs of Texp wrt ra^2
;        dT2exp_ddec2: array of 2048 2nd derivs of Texp wrt dec^2
;        dT2exp_draddec: array of 2048 2nd derivs of Texp wrt dradec
...
;        sigdT2exp_dra2: array of 2048 std uncertainties 
;        sigdT2exp_ddec2: array of 2048 std uncertainties 
;        sigdT2exp_draddec: array of 2048 std uncertainties
...
nrparams= 7   ;this is the nr of parameeters solved for
...
for np=0, 16 do xmat[ 1:6,np]= [1., delra[np], deldec[np], $
      0.5*delra[np]^2, 0.5*deldec[np]^2, delra[np]*deldec[np]]
   xmat[ 0, *] = bl
...
z17_2= {srcname:hdrsrcname[ npatt], $
        scan: hdrscan[ npatt], $
        vlsr: vlsr, $
        tcont: cont, $
        emt: a[ *, 0], $

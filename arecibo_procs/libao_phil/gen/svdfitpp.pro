; $Id: svdfit.pro,v 1.12 1997/02/04 13:11:51 stevep Exp $
; mod pjp001 n=0,ndata-1 --> n=0L,ndata-1
; mod pjp002  weight -> weights so the array matches the call. They had
;             weights=weights but used the variable weight in the routine.

FUNCTION SVDFITpp,X,Y,M, YFIT = yfit, WEIGHTS = weights, CHISQ = chisq, $
...
; <pjp002>
    ...
    ENDIF ELSE sig = FLTARR(ndata) > 1.0    
...
;
        If (reverse(size(a)))[1] eq 5 or $
           ...
           (reverse(size(sig)))[1] eq 5  then use_double=1

        IF not KEYWORD_SET(DOUBLE) THEN use_double=0 else use_double=1
...
;       of IDL.
;
        NR__SVDFIT,FUNCTION_NAME,xx,yy,ssig,ndata,A,M,COVAR,CHISQ,sing,tol,$
            DOUBLE=use_double
      
;
;       The variance is the diagonal elements of the covarianec matrix
;

...
; ------------------------------------------------------------------
; 26oct04 version 1 additions. crval2a and associated junk <pjp001>
;
    versionNum = (desc.colI.crval2a ne -1)?1:0
    version=''
    if versionNum gt 0 then begin
;       already input..
...
;
        ...
        if errmsg ne '' then goto,hdrreaderr
;
        fxbread,desc.lun,equinox,'EQUINOX',curPosStart+1,errmsg=errmsg
        if errmsg ne '' then goto,hdrreaderr
;
        fxbread,desc.lun,date_obs,'DATE-OBS',curPosStart+1,errmsg=errmsg
        if errmsg ne '' then goto,hdrreaderr
;
        fxbread,desc.lun,cunit1v,'CUNIT1V',curPosStart+1,errmsg=errmsg
        if errmsg ne '' then goto,hdrreaderr
;
        fxbread,desc.lun,ctype1v,'CTYPE1V',curPosStart+1,errmsg=errmsg
        if errmsg ne '' then goto,hdrreaderr

;      multi row  
;
        fxbread,desc.lun,crval2a,'CRVAL2A',$
            [curPosStart+1,curPosStart+rowsInRec],errmsg=errmsg
        if errmsg ne '' then goto,hdrreaderr
;
        fxbread,desc.lun,crval3a,'CRVAL3A',$
            [curPosStart+1,curPosStart+rowsInRec],errmsg=errmsg
        if errmsg ne '' then goto,hdrreaderr
;
        fxbread,desc.lun,crval2b,'CRVAL2B',$
            [curPosStart+1,curPosStart+rowsInRec],errmsg=errmsg
        if errmsg ne '' then goto,hdrreaderr
;
        fxbread,desc.lun,crval3b,'CRVAL3B',$
            [curPosStart+1,curPosStart+rowsInRec],errmsg=errmsg
        if errmsg ne '' then goto,hdrreaderr
;
;   
...
;
        ...
        if errmsg ne '' then goto,hdrreaderr
;
        fxbread,desc.lun,croff2b,'CROFF2B',$
            [curPosStart+1,curPosStart+rowsInRec],errmsg=errmsg
        if errmsg ne '' then goto,hdrreaderr
;
        fxbread,desc.lun,croff3b,'CROFF3B',$
            [curPosStart+1,curPosStart+rowsInRec],errmsg=errmsg
        if errmsg ne '' then goto,hdrreaderr
    endif
;   

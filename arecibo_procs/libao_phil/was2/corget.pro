...
;   hdronly   : if keyword is set, the only return headers. There will be
;               only be b.n.h and b.h.hf entries in the strcuture.
;   stktospc  : stokes data is returned by default as acf/xcf. Setting
;               this keyword will return spectra/cross spectra. (normalized
;               to pola+polB power.
;
;DESCRIPTION:
;
...
;
function corget, lun,b,noscale=noscale,scan=scan,sl=sl,han=han,hdronly=hdronly,$
                    stktospc=stktospc
;
...
;
    if wascheck(lun) then return,wasget(lun,b,scan=scan,han=han,$
                                  stktospc=stktospc,hdronly=hdronly)
;;    on_error,2
    on_ioerror,ioerr

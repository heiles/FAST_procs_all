...
;SYNTAX : corplot,b,wb=wb,m=pltmsk,vel=vel,rest=rest,off=ploff,pol=pol,over=over,$
...
;KEYWORDS:
;          wb:     if set then plot the wideband data
...
pro corplot,b,wb=wb,m=pltmsk,vel=vel,rest=rest,off=pltoff,pol=pol,over=over,$
            nolab=nolab,extitle=extitle,newtitle=newtitle,col=col,sym=sym,$
            chn=chn,cmask=cmask,xtitle=xtitle,ytitle=ytitle,brd=brdlist
;
...
;
    ...
    xptitle=.02
    lntitle=.8
    galdat=n_elements(b[0].b1.hf[0].g_seq) GT 0 
...
;
                        if bychn then begin
                           ...
                           x=(galdat) $ 
                            ? corfrq(b[ii].(i).hf,retvel=vel,retrest=rest,wb=usewb)$
                        ...
                        if autoscale then begin
                            if (pol eq 0) then begin
                               if usewb then begin
                                  ymin=min(b.(i).hf.g_wide*accumscl[i],max=ymax)
                               endif else begin
                                  ymin=min(b.(i).d*accumscl[i],max=ymax)
                               endelse
                            endif else begin
                               if usewb then begin
                                   ymin=min(b.(i).hf.g_wide[*,j]*accumscl[i],$
                                            max=ymax)
                               endif else begin
...
'("pwrRatio:",f4.1," ",f4.1)',b[ii].(i).h.cor.lag0pwrratio[0]*accumscl[i],$
                    ...
                    if pltoffcum ne 0. then begin
                        if not usewb  then begin
                            ...
                            oplot,x,b[ii].(i).hf.g_wide[*,j]*accumscl[i]+$
                                    pltoffcum,$
                                color=k[cind],psym=sym
                        endelse
                    endif else begin

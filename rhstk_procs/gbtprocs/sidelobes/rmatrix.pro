...
;
;OPTIONAL INPUT;
;LONGOFFSET, add this offset to the output longitudes, DEGREES
;
;OUTPUTS: 
...
;LONGOFFSET. For example, for conversion of equatorial to Galactic
...
;        the longitude offset, 57.068 deg
;
;(2) rtot is defined as COLUMN-MAJOR, which is NOT the IDL
;convention. When used in sph_coord_conv it is used as column major, so the
;matrix multiplication is done with single poind signs. This enables us to

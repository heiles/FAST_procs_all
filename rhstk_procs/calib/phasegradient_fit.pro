...
;
;PURPOSE:
;       If an angle theta (units: degrees) varies linearly with variable ff
;       (e.g., as in Faraday rotation), i.e. if
...
;
;       If you have measured values for imagpart and realpart as functions
;       of ff, and if these measured values have Gaussian noise, then this
;       nonlinear fit will provide the fit parameters CA, SA, and KK.
;
...
;
; CALLING SEQUENCE:
;       PHASEGRADIENT_FIT, frq, realpart, imagpart, dpdf_rad, 
;       coeffs, sigcoeffs, aphi, sigaphi, sigma, tfit
;
; INPUTS:
...
;       DPDF_rad: the ESTIMATE for d(phase)d(freq), i.e. the estimate for
;                 KK.  ; THE UNITS FOR DPDF ARE --->RADIANS<--- PER MHZ.
;
; OUTPUTS:
...
;       SIGMA: the mean error of the residuals. These residuals are in
;              REALPART and IMAGPART, not in the derived theta.
;       TFIT: the fitted datapoints two-element [N,2] array
;             [[REALPART],[IMAGPART]].
;
; KEYWORDS:
;       PROBLEM: nonzero if there is a problem...value depends on the
;                problem:
...
;
;       SIGTHETAFIT: the rms of the fitted angles, units ---> RADIANS <---,
;                    calculated using the angles and not xx,yy.
;
; RESTRICTIONS:
;       As with any nonlinear least squares fit, the estimate KK_GUESS
;       should be pretty good. Zero estimates for kk_guess are not
;       allowed. if the derived value of kk during the iterations exceeds 4
;       times the guessed value, it's a problem...
;
;HISTORY:
;	Written by Carl Heiles. 19 FEB 1999.
;	Corrected and updated 4 MAR 2000.
;       improved, better doc, aphi now for mean freqq not zero
;       freq. 19may08
;03may2017:
;had a problem with loop count being rezeroed so it never stopped
;because of too many loops. tried a fix, not sure if it is perfect. the
;fix is marked by ';change on 03may2017:...'
;-
...
;change on 03may2017: the following statement was moved from 5 lines
;below to here
nloop_on_5=0
...
;change on 03may2017: the following statement was moved up 5 lines
;because it never incremented aabove unity
;nloop_on_5=0
...
;';change on 03may2017:...' interchanged below
if (count  ne 0l) and (nloop_on_5 lt 50) then begin
;if (count  ne 0l) then begin
nrgood = nrgood - count
wgt[jndx]=0.
;print, 'nloop_on_5badpoints', nloop_on_5badpoints, count
;03may2017;
print, 'in count check if loop: nloop_on_5 = ', nloop_on_5
goto, iterate_on_5badpoints
endif

print, 'after count check if loop: nloop_on_5 = ', nloop_on_5

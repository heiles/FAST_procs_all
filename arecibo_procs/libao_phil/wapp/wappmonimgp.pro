...
;24jul04 - when going to next file, set position offset to zero.
;28sep04 - added pol 1234, increase cs=1.7 for 4 plots
;11apr05 - added h to menu to display header
;
function  wappmonimgp,lun,hdr,projid=projid,online=online,wapptouse=wapptouse,$
                      posrec=posrec,nrec=nrec,pol=pol,noldlut=noldlut,han=han,$
                col=col,zx=zx,zy=zy,hist=hist,flipforce=flipforce,clip=clip,$
                nsigclip=nsigclip,lvlcor=lvlcor,yymmdd=yymmdd
;     
;
    ...
    lastSize=[-1,-1]
;
;   if projid, get all the files
;
    if (n_elements(projid) eq 1)  then begin
        nwappfile=wappgetfiproj(projid,wappFileI,online=online,yymmdd=yymmdd)
        useLogFile=1
        iwapp=wapptouse-1
        if nwappfile eq 0 then begin
            print,'no wapp files found for :',projid
        endif
    endif else begin
        nwappfile=1
        useLogFile=0
...
'(" f  ",i4,"     file to use (1 to ",i3,")")',ifile+1,nwappfile)
                  ...
                  print,lab
                  print,' h           print hdr for this img'
...
'(" s  ",i4,"     step mode. 1-on,0-off")',stepmode)
                ...
                case cmd of
                   ...
                   'f': begin
                    ...
                    'l': begin
                        if useLogFile then begin
                             nwappfile=wappgetfiproj(projid,wappFileI,$
                                        online=online,yymmdd=yymmdd)
                             lab=string(format=$
...
;
        ...
        imgdisp,(img > clipLoc[0])<clipLoc[1],zx=zx,zy=zy,win=winpix,hist=hist,$
                xrange=frq,yrange=tmAr,ytitle='secs',charsize=cs
            endif else begin
        imgdisp,img,zx=zx,zy=zy,win=winpix,hist=hist,$
                xrange=frq,yrange=tmAr,ytitle='secs',charsize=cs
            endelse
         plotsLeft=numplots-(ii+1)
        endfor
        wset,winimg

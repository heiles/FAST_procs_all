...
;history:
;   17aug04 - start was2 
;   28jan05 - <pjp003> changes for fitsVersion .9
;             encTm -> utc secs was ast millisecs. 
;   05jan05 - <pjp004> coli.raj -> coli.reqraj, reqdecj
;
...
;
            ...
            slAr[itot].julday = dtemp 
            if (dtemp lt 2400000.D) then slAr[itot].julday += mjdOffset
            ...
            fxbread,desc.lun,stepname,desc.colI.scanType,rowInd+1,errmsg=errmsg
            ...
            fxbread,desc.lun,patId,desc.colI.patId,rowInd+1,$
                    errmsg=errmsg
            slAr[itot].patid   =patId
            ...
            fxbread,desc.lun,enctm,desc.colI.encTm,rowInd+1,errmsg=errmsg
            if desc.fitsVersion gt .89 then begin
                encTm=(enctm - 3600D*4)         ; utc to ast
                if encTm lt 0. then enctm=86400D + encTm ; wrap midnite
                slAr[itot].encTmSt     = encTm         ; already in secs
            endif else begin
...
;
            ...
            fxbread,desc.lun,pntErr,desc.colI.curErr,rowRange,errmsg=errmsg
...
;   ra,dec
;
            fxbread,desc.lun,ra,desc.colI.reqRaJ,rowInd+1,errmsg=errmsg
            fxbread,desc.lun,dec,desc.colI.reqDecJ,rowInd+1,errmsg=errmsg
            slAr[itot].raHrReq=(desc.fitsVersion lt .89)?ra       $
                                                       :ra/15D ; <pjp003>
            slAr[itot].decDReq=dec
;
;  get ra,dec deltas, start,end of scan
;
            fxbread,desc.lun,raEnd,desc.coli.reqRaJ,$
                rowInd+desc.scanI[iscan].rowsInScan,errmsg=errmsg
            fxbread,desc.lun,decEnd,desc.coli.reqDecJ,$
                rowInd+desc.scanI[iscan].rowsInScan,errmsg=errmsg
            slAr[itot].decDelta=(decEnd-dec)*60.
            if (slAr[itot].decDelta) lt 1e-6  then slAr[itot].decDelta=0.
            toAmin=(desc.fitsVersion lt .89) ?60.*15. : 60. ;<pjp003>  

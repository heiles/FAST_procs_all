...
; nsigclp[2]: float if supplied then compute the clipping levels using a 
;                robust estimator of rms (median, throw out outliers, repeat).
;				 Data will be clipped to [-nsigClip[0],nsigClip[1]]*sig after removal
;                of mean. This overrides clip keyword.
;                if nsigclip is a single element then clip to
;                [-sig,sig]*nsigclip.
;                if nsigclip[0] is negative, force it positive.
...
pro imgdisp,d,ret,zx=zx,zy=zy,histeq=histeq,rdpix=rdpix,win=win,$
	...
	maxiter=maxiter,poswin=poswin,chkinf=chkinf,double=double
...
;
	...
	if (useNsigclip) then begin
		if n_elements(nsigClip) eq 1 then begin
			nsigClipL=[-nsigClip,nsigClip]
		endif else begin
			nsigClipL=[-abs(nsigClip[0]),nsigClip[1]]
		endelse
	endif
    ...
    if (zx ne 1) or (zy ne 1) or (useNsigClip) or (useClip) then begin
        if keyword_set(histeq) then begin
            ...
            if invert then  begin
				if useNsigClip then begin
					a=meanrob(d,sig=sig,maxiter=maxiterL,chkinf=chkinf,double=double)
					minval=nsigclipL[0]*sig - a
					maxval=nsigclipL[1]*sig - a
				endif else begin
...
;
				   ...
				   case 1 of
				     useNsigClip: begin
					  	a=meanrob(ret,sig=sig,maxiter=maxiterL,chkinf=chkinf,double=double)
					  	minval=nsigclipL[0]*sig - a
					  	maxval=nsigclipL[1]*sig - a
						end
				      useClip:begin

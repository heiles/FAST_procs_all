pro gfitcfix, look, xdata, tdata, zro0, hgt0, cen0, wid0, tfit, sigma, $
...
s = fltarr(2*ngaussians+1, datasize)
...
resid = tdata - tfit
sigsq = total( resid^2)/(datasize - 2.*ngaussians - 1.0)
sigma = sqrt( sigsq)
sigarray = sigsq * ssi[indgen(2*ngaussians+1)*(2*ngaussians+2)]
sigarray = sqrt( sigarray)

sigzro1 = sigarray[ 0]
sighgt1 = sigarray[ 2*indgen(ngaussians) + 1]
sigcen1 = 0.0
sigwid1 = sigarray[ 2*indgen(ngaussians) + 2]/0.6005612

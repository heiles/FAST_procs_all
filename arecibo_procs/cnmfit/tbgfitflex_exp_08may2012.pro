pro tbgfitflex_exp_08may2012, look, xdataa, tdataa, xindxrange, $
...
;
;PURPOSE:
;    Fit multiple emission/absorption gaussians.
...
;     hgtcnm: the array of N estimated central opacity of the CNM Gaussians.
...
;     hgtcnmyn: array of N 0 or 1; 0 does not fit the hgt, 1 does.
...
;
;     zrownm: the estimated continuum temp from beind all the CNM
;       NOTE: this used to be: constant zero offset of the WNM opacity.
;     hgtwnm: the array of N estimated central opacity of the WNM Gaussians.
...
;
;     zrwnmyn: if 0, does not fit zero level; if 1, it does.
...
;     hgtcnm1: the array of N fitted CNM central opacities;
...
;     sighgtcnm1: the array of errors of the CNM heights; zero if hgtcnmyn=0).
...
;     zrownm1: fitted zero offset for WNM opacity (held constant if zrownmyn=0).
...
;     sigzrownm1: the error of the fitted WNM zero offset; zero if zrownmyn=0.
...
;      halfasseduse=halfasseduse, mult fctr for corrections; defauult is 
;
;RESTRICTIONS:
...
ngaussians_cnm = n_elements(hgtcnm)
ngaussians_wnm = n_elements(hgtwnm)

;DETERMINE THE NR OF PARAMETERS TO FIT...
nparams = total(zrocnmyn) + total(hgtcnmyn) + total(cencnmyn) + $
	total(widcnmyn) + total(tspincnmyn) + $ 
	total(zrownmyn) + total(hgtwnmyn) + total(cenwnmyn) + $
...
if (hgtcnmyn[ng] ne 0) then begin 
	sfull_to_s[scol]=sfullcol
	scol=scol+1 
endif
sfullcol = sfullcol + 1
...
del = 0.0000025D0
hgtcnm1[ng] = hgtcnm1[ng] + del
tb_exp, xdata, $
	zrocnm1, hgtcnm1, cencnm1, widcnm1/dp600, tspincnm1, ordercnm, $
        zrownm1, hgtwnm1, cenwnm1, widwnm1/dp600, fwnm1, $
        tb_cont, tb_wnm_tot, tb_cnm_tot, tb_totplus, exp_tau_sum
hgtcnm1[ng] = hgtcnm1[ng] -2.* del
tb_exp, xdata, $
	zrocnm1, hgtcnm1, cencnm1, widcnm1/dp600, tspincnm1, ordercnm, $
        zrownm1, hgtwnm1, cenwnm1, widwnm1/dp600, fwnm1, $
        tb_cont, tb_wnm_tot, tb_cnm_tot, tb_totminus, exp_tau_sum
...
tb_exp, xdata, $
	zrocnm1, hgtcnm1, cencnm1, widcnm1/dp600, tspincnm1, ordercnm, $
        zrownm1, hgtwnm1, cenwnm1, widwnm1/dp600, fwnm1, $
        tb_cont, tb_wnm_tot, tb_cnm_tot, tb_totplus, exp_tau_sum
zrownm1 = zrownm1 - 2.*del
tb_exp, xdata, $
	zrocnm1, hgtcnm1, cencnm1, widcnm1/dp600, tspincnm1, ordercnm, $
        zrownm1, hgtwnm1, cenwnm1, widwnm1/dp600, fwnm1, $
        tb_cont, tb_wnm_tot, tb_cnm_tot, tb_totminus, exp_tau_sum
zrownm1 = zrownm1 + del
zrownmder = (tb_totplus - tb_totminus)/(2.*del)
...
adelt = abs(delt)
adelt = 0.2*abs(hgtcnm1) < adelt
delthgtcnm = adelt*(1.- 2.*(delt lt 0.))
...
hgtcnm1 = hgtcnm1 + halfassed * delthgtcnm
...
sighgtcnm1 = sigarraya[ 4*indgen(ngaussians_cnm) + 1]

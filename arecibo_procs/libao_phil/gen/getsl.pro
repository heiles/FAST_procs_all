...
;08dec02 - added julday. this is the start scan time.
;
...
;
        if (hdr.std.scannumber ne curscan) then begin
            ...
            sl[i].stat=0 
            year =hdr.std.date/1000L
            dayno=hdr.std.date mod 1000L
            dm=daynotodm(dayno,year)
            sl[i].julday=julday(dm[1],dm[0],year,0.D,0.D,0.D) + $
                    hdr.std.stScanTime/86400.D + 4.D/24.D     ;to gmt
            sl[i].rectype=0
...
;                  coron routine
;
                   (sl[i].procname eq 'on') : begin
                       if string(hdr.proc.car[*,0]) eq 'on' then $
                            sl[i].rectype=5
                    end
;
;                  cormap1
;
                   (sl[i].procname eq 'cormap1') : sl[i].rectype=6
;
;                  cormapdec
;
                   (sl[i].procname eq 'cormapdec') : sl[i].rectype=7
;
;                  cordrift
;
                   (sl[i].procname eq 'cordrift') : sl[i].rectype=8
;
;                  corcrossch
;
                   (sl[i].procname eq 'corcrossch') : sl[i].rectype=9
;
;                  corcrosschL
;
                   (sl[i].procname eq 'corcrosschL') : sl[i].rectype=13
;
;                  x111auto rfi monitoring
;
                   (sl[i].procname eq 'x111auto') : sl[i].rectype=10
;
;                  one murrays on , then calon/off at the null
;
                   (sl[i].procname eq 'one') : sl[i].rectype=11
;
;                  onoffbml . position on , off in null  then calonoff
;
                   (sl[i].procname eq 'onoffbml') : sl[i].rectype=12
                   else : sl[i].rectype=0
                endcase
            endelse
            if gotdop then begin

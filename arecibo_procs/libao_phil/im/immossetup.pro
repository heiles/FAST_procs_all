...
; setup for mosaics
;
;  code:  1=dailyaverage, 2=time/freq img, 3=rms/mean
;  hardcopy: 0 to screen, 1 -> to file filename, < 0 assume continueing
;            call to previously openned file
...
; keywords:
;  file         : if no supplied then idl.ps
...
;       psYsize   = 5.          ; 9 inches for mela
        mos.charSizeTsys=.86
		if (hardcopy gt 0) or (hardcopy eq -1)  then begin

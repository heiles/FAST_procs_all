...
pro masplotmb,b,vel=vel,rest=rest,off=pltoff,pol=pol,over=over,$
            ...
            chn=chn,xtitle=xtitle,ytitle=ytitle,bmlist=bmlist,$
...
;
;
    if (n_elements(bmlist) ne 0) && (bmlist ge 0)  then begin
        itemp=long(bmlist)
        plttmp=0L
        while itemp ge 0 do begin
            ival=itemp mod 10
            if ival ge 0 then plttmp=plttmp or ( ishft(1,ival))
            itemp=(itemp le 9)?-1:itemp/10L
        endwhile
    endif else begin
        plttmp='ff'x
    endelse

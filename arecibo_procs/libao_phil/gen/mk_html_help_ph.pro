; $Id: mk_html_help.pro,v 1.16 2001/04/04 20:40:26 ali Exp $
;
; Copyright (c) 1995-2001, Research Systems, Inc.  All rights reserved.
;       Unauthorized reproduction prohibited.
; pjp stole from rsi made some mods, appended _ph to the names
; <pjp001>  path_sep new after 5.4
; <pjp002>  added bgcolor keyword
; <pjp003>  got rid of lines talking about mk_help_html at the beginng
;+
; NAME:
;   MK_HTML_HELP_PH
;
...
;----------------------------------------------------------------------------
PRO  mhh_grab_hdr_ph,name,dict,infile_indx,libfile_indx,txt_file,verbose,$
     strict
;
...
;
 ...
 WHILE (1) DO BEGIN
  ...
  IF (found) THEN BEGIN
   ...
   IF (strict) THEN mhh_strict_ph,header
...
; <pjp001>
    ...
    IF (strict) THEN mhh_strict_ph, ifname
...
;----------------------------------------------------------------------------
PRO mhh_gen_file_ph,dict,txt_file,infiles,libfiles,outfile,verbose,title,$
        strict,bgcolor=bgcolor
;
...
;
 ...
 PRINTF,final_file,'<!-- This file was generated by mk_html_help_ph.pro -->'
 ...
 ; <pjp002>
 if bgcolor ne '' then begin
    PRINTF,final_file,'<body bgcolor='+bgcolor+'>'
 endif else begin
 ...
 FOR i=0,count-1 DO BEGIN
  ...
  IF (entry.nline GT 0) THEN BEGIN
   ...
   ENDIF ELSE BEGIN
    ...
    IF (strict) THEN mhh_strict_ph,fname
...
;----------------------------------------------------------------------------
PRO mk_html_help_PH, sources, outfile, VERBOSE=verbose, TITLE=title,$
            STRICT=strict,bgcolor=bgcolor
 ...
 FOR i=0,count-1 DO BEGIN
  ...
  IF (istlb[i]) THEN BEGIN
   ...
   FOR j=0, lib_count-1 DO BEGIN
    ...
    mhh_grab_hdr_ph,files[j],dict,i,lib_total+j,txt_file,verbose,strict
   ENDFOR
  ENDIF ELSE BEGIN
   name = infiles[i]
   mhh_grab_hdr_ph,name,dict,i,-1L,txt_file,verbose,strict
  ENDELSE
 ENDFOR

 ; Generate the HTML file.
 if not keyword_set(bgcolor) then bgcolor=''
 mhh_gen_file_ph,dict,txt_file,infiles,libfiles,outfile,verbose,title,strict,$
        bgcolor=bgcolor
END

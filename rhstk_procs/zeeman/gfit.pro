...
pro gfit, look, xdata, tdata, zro0, hgt0, cen0, wid0, tfit, sigma, $
    zro1, hgt1, cen1, wid1, sigzro1, sighgt1, sigcen1, sigwid1, problem, cov, $
...
;       NLOOPS: the nr of loops it used for convergence.
;
;OUTPUTS:
...
s = fltarr(3*ngaussians+1, datasize)
...
for ng = 0, ngaussians-1 do begin
    ...
    s[(3*ng+1), *] = sum1          ;HGT
    s[(3*ng+2), *] = sum2*sum6     ;CNTR
    s[(3*ng+3), *] = sum3*sum6     ;WIDTH
...
if (nloop ge nloopmax-1) then begin
    if keyword_set( quiet) eq 0 then print, nloopmax, ' loops; halfassed = ', halfassed
    if keyword_set( quiet) eq 0 then print, 'TRY A SMALLER VALUE OF HALFASSED...MAYBE 0.2'
...
resid = tdata - tfit
sigsq = total( resid^2)/(datasize - 3.*ngaussians - 1.0)
sigma = sqrt( sigsq)
sigarray = sigsq * ssi[indgen(3*ngaussians+1)*(3*ngaussians+2)]
...
doug = ssi[indgen(3*ngaussians+1)*(3*ngaussians+2)]
doug = doug#doug
cov = ssi/sqrt(doug)

...
function corinpscan,lun,b,brecs,sum=sum,maxrecs=maxrecs,scan=scan,$
                    sl=sl,dbg=dbg,han=han,df=df,_extra=e
;
;
    ...
    if maxrecs le 0 then maxrecs=501
...
; 
      ...
      istat=corget(lun,bl,han=han,_extra=e)
...
;
      ...
      if bl.(0).h.std.scannumber ne scan then begin
        hitNewScan=1
        goto,done
      endif
      if dosum then begin
    ...
    if (grp ne limit) and (not hitEof) then point_lun,lun,curpos
    if dosum then begin
        scl=1./grp
        for i=0,nbrds-1 do begin
                b.(i).d=b.(i).d*scl
    ...
    if (not dosum) and (not hitNewScan) and (not hitEof) and $
        (not maxRecsSet) then begin
        nrecs=n_elements(b)
        print,$
'Warning.. corinpscan found',nrecs,' recs.It did not hit the end of scan'
        print,$
'  ..Use maxrecs= keyword to increase the default number of records to read' 
    endif
    return,1
errinp:
    return,0

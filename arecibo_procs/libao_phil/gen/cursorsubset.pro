...
;        
;04may06: fix so that is works if xaxis is decreasing
function cursorsubset,xaxis,indArr
;
    maxset=100
    indArrLoc=fltarr(2,maxset)
    nptsx=(size(xaxis))[1]
    xdecrease=xaxis[0] gt xaxis[nptsx-1]
    done=0
    setcnt=0
    lnstyle=2
...
;           print,xp1,yp1,!mouse.button
        ...
        endwhile
got1:
        ind=(xdecrease)? where(xp1 lt xaxis,count):where(xp1 gt xaxis,count)
        print,'dbg:', count
        case count of
            nptsx:ind1=nptsx-1
            0    :ind1=0
...
;
        ind=(xdecrease)? where(xp2 lt xaxis,count):where(xp2 gt xaxis,count)
        case count of
            0    :ind2=0
            nptsx:ind2=nptsx-1
             else:begin
        ...
        endcase
        if (((ind1 ge ind2) and (not xdecrease)) or $
            ((ind1 le ind2) and (xdecrease))) then   begin

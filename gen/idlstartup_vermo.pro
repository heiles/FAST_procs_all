...
print, 'STARTUP FILE IS $~/dzd2/heiles/idl/gen/idlstartup_vermo.pro'
...
print, 'loading ARECIBO coordinates into COMMON ANGLESTUFF'
...
;retain=1 ;system governs indows; retain=2 means IDL governs 
retain=2 ;IDL governs indows
repeat begin &$
   print, format = $
'($,"<g>ray, <p>seudo, pseudo<2>56, <t>rue, <d>irect, <n>othing, or <s>ystem: ")' &$
   mode = strlowcase(get_kbrd(1)) & print &$
   case (mode) of &$
     'g' : device, Gray_Scale=8,    retain=retain &$     ; GRAYSCALE
     'p' : device, Pseudo_Color=8,  retain=retain &$     ; PSEUDOCOLOR
     '2' : device, Pseudo_Color=8,  retain=retain &$     ; PSEUDOCOLOR 256
     't' : device, True_Color=24,   retain=retain &$     ; TRUECOLOR
     'd' : device, Direct_Color=24, retain=retain &$     ; DIRECTCOLOR
...
;if ( mode ne 'n') then if strmatch(getenv('OSTYPE'),'linux') then device, /INS;TALL_COLORMAP
...
if ( mode ne 'n') then window, 0, xsize=300, ysize=225
;if ( mode ne 'n') then window, 1, xsize=300, ysize=225
